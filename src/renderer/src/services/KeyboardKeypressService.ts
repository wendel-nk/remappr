...
export class KeyboardKeypressService {
    ...
    mapKeyCodeToPosition(
        ...
        if (
            !config.layouts ||
            !config.keymap?.layers?.[config.selectedLayerIndex]
        ) {
            return null
        }
        ...
        for (
            let i = 0;
            i < currentLayer.bindings.length && i < currentLayout.keys.length;
            i++
        ) {
        ...
        if (expectedName) {
            for (
                let i = 0;
                i < currentLayer.bindings.length &&
                i < currentLayout.keys.length;
                i++
            ) {
                const binding = currentLayer.bindings[i]
